...
router.post('/add', ensureAuth, async (req, res) => {
  if (req.body.name.length < 5) {
    ...
    if (cubes.length < 24) {
      ...
      cube.decks = [];
      cube.articles = [];
      cube.image_uri = carddb.cardFromId(carddb.nameToId['doubling cube'][0]).art_crop;
      cube.image_name = carddb.cardFromId(carddb.nameToId['doubling cube'][0]).full_name;
      cube.image_artist = carddb.cardFromId(carddb.nameToId['doubling cube'][0]).artist;
