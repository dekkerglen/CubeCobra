...
describe('createPacks', () => {
  beforeEach(() => {
    ...
    //Card ids were randomly generated by typing (from a set of 0 to 29, assuming 30 cards are needed)
    cubeCardIdsByFilter = {
...
let cubeCardIdsByFilter: Record<string, number[]>;
...
const getFilteredCardGenerator = (orderOfUnfilteredCardGrabbing: string[]) => {
  return (cardFilters: string[]): DraftResult => {
    ...
    if (cardFilterString === '') {
      ...
      cardSet = cubeCardIdsByFilter[nextUnfilteredCardGroupToPickFrom];
      if (cardSet.length === 0) {
        return {
          card: undefined,
      ...
      cardSet = cubeCardIdsByFilter[cardFilterString];
      if (cardSet.length === 0) {
        return {
          card: undefined,
