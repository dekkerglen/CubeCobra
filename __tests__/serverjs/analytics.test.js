...
describe('analytics', () => {
  ...
  test('GetTokens returns the correct tokens', () => {
    expect.hasAssertions();
    var cardJSON = fs.readFileSync(cardsFixturePath);
    var cards = JSON.parse(cardJSON);
    updatecards.initializeCatalog();
    return updatecards.saveAllCards(cards).then(function() {
      return carddb.initializeCardDb('private', true).then(function() {
        const result = analytics.GetTokens(cubefixture.exampleCube.cards, carddb);
        // guarantee that each source card's tokens share ids with the tokens returned.
        // not all of the token-producing cards are in the small fixtures DB, so some sourceCards
        // will be missing.
        result.forEach(([token, sourceCards]) => {
          sourceCards.forEach(sourceCard => {
            if (sourceCard.tokens !== undefined) {
              const sourceCardTokenIds = sourceCard.tokens.map(card => card.tokenId);
              expect(sourceCardTokenIds).toContain(token._id);
            }
          });
        });

        const castleArvendaleCardId = '6f1383eb-aa7d-4d3b-bee4-8cffba9ae846';
        // a cube of only castle arvendale
        const castleArvendaleCube = [
          {
            tags: ['New'],
            colors: ['W'],
            status: 'Not Owned',
        ...
        const whiteHumanTokenCardId = '94057dc6-e589-4a29-9bda-90f5bece96c4';
        const generatedCastleArvendaleTokens = analytics.GetTokens(castleArvendaleCube, carddb);
        const expectedCastleArvendaleToken = carddb.cardFromId(whiteHumanTokenCardId);
        // it makes one token
        expect(generatedCastleArvendaleTokens).toHaveLength(1);
        expect(generatedCastleArvendaleTokens[0][0]).toHaveProperty('_id', whiteHumanTokenCardId);
      });
    });
  });
});
