...
const getTokens = (card, catalogCard) => {
...
const saveCard = async (newcard, isExtra) => {
  let card = await Card.findById(newcard.scryfall_id).lean();
...
async function saveEnglishCard(card) {
  if (card.layout === 'transform') {
    await saveCard(convertCard(card, true), true);
  }
  await saveCard(convertCard(card));
}
