...
const { CARD_STATUSES, PrintFilter } = require('../../datatypes/Card');
...
router.post('/add', ensureAuth, recaptcha, async (req, res) => {
  try {
    ...
    const cube = {
      ...
      defaultPrinting: PrintFilter.RECENT,
      disableAlerts: false,
      basics: [
...
router.post('/updatesettings/:id', ensureAuth, async (req, res) => {
  try {
    ...
    if (![PrintFilter.RECENT, PrintFilter.FIRST].includes(defaultPrinting)) {
