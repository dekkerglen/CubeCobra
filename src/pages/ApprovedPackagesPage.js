...
const ApprovedPackagesPage = ({ loginCallback, items, lastKey }) => {
  ...
  const fetchMoreData = useCallback(async () => {
    ...
    const response = await csrfFetch(`/packages/getmoreapproved`, {
      method: 'POST',
      headers: {
  ...
  }, [filter, currentLastKey, sortDirection, sort, packages]);
      ...
      <Card>
        ...
        <Nav tabs>
          <NavItem className="ms-2 clickable">
            <NavLink active>Approved</NavLink>
          </NavItem>
          <NavItem className="ms-2 clickable">
            <NavLink href="/packages/submitted">Submitted</NavLink>
          </NavItem>
          <NavItem className="ms-2 clickable">
            <NavLink href="/packages/user">Your Packages</NavLink>
          </NavItem>
        </Nav>
        <CardBody>
          <InfiniteScroll dataLength={packages.length} next={fetchMoreData} hasMore={currentLastKey != null}>
            {packages.map((pack) => (
              <Row className="mx-0">
