...
class SortCollapseRaw extends Component {
  ...
  handleSave() {
    ...
    csrfFetch("/cube/api/savesorts/" + $('#cubeID').val(), {
      ...
      this.addAlert('success', 'Default sorts saved.')
    ).catch(err =>
      this.addAlert('danger', 'Error saving default sorts.')
    );
  }
      ...
      <Collapse {...props}>
        <Container>
          ...
          <Row className="mb-3">
            {!canEdit ? '' :
              <Col>
                <Button color="success" onClick={this.handleSave}>Save as Default Sort</Button>
              </Col>
            }
          </Row>
        </Container>
