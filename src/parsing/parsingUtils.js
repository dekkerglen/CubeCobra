import { createTokenInstance, createToken, Alternation, Flat, Option, Repetition, RepetitionMandatory, Terminal } from 'chevrotain';
...
export function consumeOneOf(words) {
  return new Alternation({ definition: Array.prototype.map.call(words, (w) => new Flat({ definition: consumeWord(w) })) });
}
      ...
      switch(regex[i]) {
        ...
        case '+':
          if (lastGroup !== null) {
            ...
            operations[operations.length - 1] = new RepetitionMandatory({ definition: [operations[operations.length - 1]] });
          } else {
            throw new Error(`Nothing to repeat for + in "${regex}" at position ${i}`);
          }
          break;
        ...
        case '|':
          ...
          operations = [new Alternation({ definition: [new Flat({ definition: [...operations] }), new Flat({ definition: internalCall.operations })] })];
          i = internalCall.endIndex;
...
export default { getTokenType, tokenize, consumeWord, consumeOneOf, consumeLetter, consumeNumber, consumeRegex, tokenTypes };
