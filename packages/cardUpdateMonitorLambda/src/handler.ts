...
import { cardUpdateTaskDao, exportTaskDao } from '@server/dynamo/daos';
...
/**
 * Start an ECS task to run the export job
 */
async function startExportTask(taskId: string): Promise<{ taskArn: string; success: boolean }> {
  const clusterName = process.env.ECS_CLUSTER_NAME;
  const taskDefinitionArn = process.env.ECS_TASK_DEFINITION_ARN;
  const subnetIds = process.env.ECS_SUBNET_IDS?.split(',') || [];
  const securityGroupIds = process.env.ECS_SECURITY_GROUP_IDS?.split(',') || [];
...
/**
 * Export task monitoring function
 */
export async function monitorExportTasks(): Promise<void> {
  ...
  // Double-check that no ECS tasks are currently running (safety check)
  const isRunning = await isCardUpdateTaskRunning(); // Reusing same check for concurrency
  if (isRunning) {
    console.log('ECS task is already running - aborting to maintain max concurrency of 1');
    return;
  }
  ...
  // Start the ECS task
  const { taskArn, success } = await startExportTask(newTask.id);
...
export const handler = async (event: any) => {
  console.log('Card update and export monitor triggered', { event });
