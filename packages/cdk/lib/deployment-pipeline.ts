        ...
        phases: {
          ...
          build: {
            commands: [
              ...
              'export JOBS_ECR_REPO=$(aws cloudformation describe-stacks --stack-name CubeCobraBetaStack --query "Stacks[0].Outputs[?contains(OutputKey, \'JobsEcrRepositoryUri\')].OutputValue" --output text)',
        ...
        phases: {
          ...
          build: {
            commands: [
              ...
              'cd ../..',
              'echo Building and pushing jobs Docker image...',
              'export JOBS_ECR_REPO=$(aws cloudformation describe-stacks --stack-name CubeCobraProdStack --query "Stacks[0].Outputs[?contains(OutputKey, \'JobsEcrRepositoryUri\')].OutputValue" --output text)',
              'echo Jobs ECR Repository: $JOBS_ECR_REPO',
              "export ECR_REGISTRY=$(echo $JOBS_ECR_REPO | cut -d'/' -f1)",
              'aws ecr get-login-password --region $AWS_DEFAULT_REGION | docker login --username AWS --password-stdin $ECR_REGISTRY',
              'docker build -t cubecobra-jobs:$CODEBUILD_RESOLVED_SOURCE_VERSION -f packages/jobs/Dockerfile .',
