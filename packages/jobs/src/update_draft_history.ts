...
export const adjustElo = (winnerElo: number, loserElo: number, kFactor: number): [number, number] => {
  ...
  // Expected score for loser = 1 / (1 + 10^((winnerElo - loserElo) / 400))
  ...
  (async () => {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (exists) {
        ...
        // Key format is YYYY-MM-DD with 1-indexed month, Date constructor expects 0-indexed
        ...
        for (const batch of logRowBatches) {
          const drafts: DraftType[] = await draftDao.batchGet(
            batch.filter((item) => item.complete).map((row) => row.id),
          );
