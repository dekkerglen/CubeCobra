...
const getPickColors = (oracleId: string, cardMeta: Record<string, CardMeta>): string[] => {
  const identity = (cardMeta[oracleId]?.colorIdentity ?? []).filter(
    (color) => MTG_COLORS[color] !== undefined && color !== 'M',
  );
...
const PickColorSpectrum: React.FC<{
  ...
  return (
    <div className="flex w-full overflow-hidden rounded border border-black/10 bg-bg-accent" style={{ height }}>
      {packs.map((pack, packIdx) => {
        ...
        return (
          <div
            key={pack.packNumber}
            className={['relative flex h-full', clickable ? 'cursor-pointer' : ''].join(' ')}
            style={{
              width: `${widthPct}%`,
            ...
            title={
              clickable ? `Pack ${pack.packNumber + 1} — click to ${isSelected ? 'collapse' : 'expand'}` : undefined
            }
          >
          ...
          {pagedPools.map((summary) => {
            ...
            const firstColorAnchorImage = summary.firstColorAnchorPick?.imageUrl
              ? {
                  ...
                  imageUrl: summary.firstColorAnchorPick.imageUrl.replace('/normal/', '/art_crop/'),
                }
              : null;
            const secondColorBridgeImage = summary.secondColorBridgePick?.imageUrl
              ? {
                  ...summary.secondColorBridgePick,
                  imageUrl: summary.secondColorBridgePick.imageUrl.replace('/normal/', '/art_crop/'),
                }
              : null;
            return (
              <button
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-start gap-2 flex-wrap">
                        {firstColorAnchorImage && (
                          <div
                            key={firstColorAnchorImage.oracle_id}
                            className="flex-shrink-0 overflow-hidden rounded"
                            style={{ width: 48, height: 48 }}
                            title={`P${firstColorAnchorImage.packNumber + 1}P${firstColorAnchorImage.pickNumber} · ${firstColorAnchorImage.name}`}
                          >
                            <img
                              src={firstColorAnchorImage.imageUrl}
                              alt={firstColorAnchorImage.name}
                        ...
                        {secondColorBridgeImage && (
                          <div
                            key={secondColorBridgeImage.oracle_id}
                            className="flex-shrink-0 overflow-hidden rounded"
                            style={{ width: 48, height: 48 }}
                            title={`P${secondColorBridgeImage.packNumber + 1}P${secondColorBridgeImage.pickNumber} · ${secondColorBridgeImage.name}`}
                          >
                            <img
                              src={secondColorBridgeImage.imageUrl}
                              alt={secondColorBridgeImage.name}
                              className="w-full h-full object-cover object-center"
                            />
                          </div>
                        )}
            ...
            <tbody className="divide-y divide-border/50">
              {pagedPools.map((summary) => {
                ...
                const openingPickImages = summary.openingPicks
                  .filter((c) => c.imageUrl)
                  .map((c) => ({ ...c, imageUrl: c.imageUrl.replace('/normal/', '/art_crop/') }));
                const firstColorAnchorArt =
                  summary.firstColorAnchorPick?.imageUrl?.replace('/normal/', '/art_crop/') ?? '';
                ...
                const secondColorBridgeArt =
                  summary.secondColorBridgePick?.imageUrl?.replace('/normal/', '/art_crop/') ?? '';
                const expanded =
                  viewMode === 'draft' && expandedPack !== null && expandedPack.poolIndex === summary.pool.poolIndex;
                return (
                              ...
                              >
                                ...
                                <div className="flex flex-col min-w-0">
                                  ...
                                  <span className="text-xs text-text-secondary tabular-nums">
                                    P{summary.firstColorAnchorPick.packNumber + 1}P
                                    {summary.firstColorAnchorPick.pickNumber}
                                  </span>
                                </div>
                              </div>
                            ) : (
                            ...
                            {summary.secondColorBridgePick ? (
                              ...
                              >
                                ...
                                <div className="flex flex-col min-w-0">
                                  ...
                                  <span className="text-xs text-text-secondary tabular-nums">
                                    P{summary.secondColorBridgePick.packNumber + 1}P
                                    {summary.secondColorBridgePick.pickNumber}
                                  </span>
                                </div>
                              </div>
                            ) : (
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex items-center justify-between">
                                  <Text xs semibold className="uppercase tracking-wide text-text-secondary">
                                    Pack {expandedPack!.packNumber + 1} · Draft {summary.pool.draftIndex + 1} · Seat{' '}
                                    {summary.pool.seatIndex + 1}
                                  </Text>
                                  <button
                                    type="button"
                                    onClick={(e) => {
