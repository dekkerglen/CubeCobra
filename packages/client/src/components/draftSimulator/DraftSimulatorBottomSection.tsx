...
import type { DraftSimulatorBottomTab } from '../../hooks/draftSimulatorHookTypes';
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {activeFilterPoolIndexSet && (
                <button
                  ...
                  onClick={() =>
                    downloadDraftBreakdownCsv(
                      displayedPools.filter((p) => activeFilterPoolIndexSet.has(p.poolIndex)),
                      'filtered',
                    )
                  }
          ...
          <Col xs={12} md={6}>
            <div className="h-full rounded border border-border bg-bg">
              <div className="border-b border-border bg-bg-accent/50 px-3 py-2 flex flex-col gap-0.5">
                ...
                <Text xs className="text-text-secondary">
                  Cards most often left in the sideboard across matching pools.
                </Text>
              </div>
                        ...
                        return (
                          <tr key={entry.oracle_id} className="hover:bg-bg-accent/40">
                            <td className="px-3 py-2 font-medium text-text truncate">
                              {meta
                                ? renderAutocardNameLink(entry.oracle_id, meta.name, meta.imageUrl)
                                : entry.oracle_id}
                            </td>
            ...
            <div className="h-full rounded border border-border bg-bg">
              <div className="border-b border-border bg-bg-accent/50 px-3 py-2 flex items-start justify-between gap-3">
                <div className="flex flex-col gap-0.5">
                  ...
                  <Text xs className="text-text-secondary">
                    Pairs of cards most often drafted together into the same deck.
                  </Text>
                </div>
                    ...
                    <thead className="bg-bg-accent/50 border-b border-border">
                      <tr>
                        ...
                        <th
                          ...
                          title="% of decks containing both cards"
                        >
                          % decks
                        </th>
                        <th
                          className="px-3 py-2 text-right text-xs font-semibold text-text-secondary"
                          title="% of decks with the rarer card that also have the other"
                        >
                          Given rarer
                        </th>
                      </tr>
                      ...
                      {topCardPairings.map((entry) => {
                        ...
                        return (
                          <tr key={`${entry.oracle_id_a}|${entry.oracle_id_b}`} className="hover:bg-bg-accent/40">
                            ...
                            <td className="px-3 py-2 font-medium text-text truncate">
                              {metaA
                                ? renderAutocardNameLink(entry.oracle_id_a!, metaA.name, metaA.imageUrl)
                                : entry.oracle_id_a}
                            </td>
                            <td className="px-2 py-1">
                              {artB && (
                            ...
                            <td className="px-3 py-2 font-medium text-text truncate">
                              {metaB
                                ? renderAutocardNameLink(entry.oracle_id_b!, metaB.name, metaB.imageUrl)
                                : entry.oracle_id_b}
                            </td>
                            <td className="px-3 py-2 text-right text-text-secondary">{entry.count}</td>
                            <td className="px-3 py-2 text-right text-text-secondary">
                              {(entry.rawPct * 100).toFixed(1)}%
                            </td>
                            <td className="px-3 py-2 text-right text-text-secondary">
                              {(entry.pct * 100).toFixed(1)}%
                            </td>
          ...
          <CardHeader>
            <Flexbox direction="row" justify="between" alignItems="center" className="flex-wrap gap-2">
              <div className="flex flex-col gap-1">
                ...
                <Text xs className="text-text-secondary">
                  Per-color rollup of pick, pool, and deck signals — useful for judging which colors are over- or
                  under-picked relative to expectation.
                </Text>
              </div>
            </Flexbox>
          </CardHeader>
