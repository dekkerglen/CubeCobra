    ...
    <div className="flex flex-col gap-5">
          ...
          <div className="min-w-0 flex-1 pt-2">
            <div>
              <Text semibold className="text-lg leading-snug">
                {selectedCardInfo ? (title ? `${selectedCardInfo.name} in ${title}` : selectedCardInfo.name) : title}
              </Text>
            </div>
            {subtitle ? (
              <div className="mt-1">
            ...
            {selectedCardInfo &&
              (selectedCardInfo.pickRate !== undefined || (selectedCardInfo.avgPickPosition ?? 0) > 0) && (
                <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                  {selectedCardInfo.pickRate !== undefined && (
                    <span className="text-sm text-text-secondary/50">
                      Pick rate{' '}
                      <span className="text-text-secondary/80">{(selectedCardInfo.pickRate * 100).toFixed(1)}%</span>
                    </span>
                  )}
                  {(selectedCardInfo.avgPickPosition ?? 0) > 0 && (
                    <span className="text-sm text-text-secondary/50">
                      Avg pos{' '}
                      <span className="text-text-secondary/80">{selectedCardInfo.avgPickPosition!.toFixed(1)}</span>
                    </span>
                  )}
                </div>
              )}
      ...
      {matchingCount === 0 ? (
        <Text sm className="text-text-secondary">
          No pools match all active filters. Try removing a card or changing the scope.
        </Text>
      ) : (
        <>
                  ...
                  {selectedCardInfo.cardImages.map((img) => (
                    ...
                    >
                      <img
                        src={img.imageUrl}
                        alt={img.name}
                        className="w-full rounded border-2 border-primary shadow-sm"
                      />
                    </AutocardLink>
                  ))}
                </div>
              ...
              {commonCardList.length > 0 && (
                <div>
                  <Text xs className="text-text-secondary font-medium uppercase tracking-wider mb-1.5">
                    Most common cards in matching pools
                  </Text>
                  <div className="grid grid-cols-6 gap-1.5">
    ...
    <Card className="border-border">
            ...
            <div className="flex flex-row items-center gap-2">
              ...
              <div className="inline-flex rounded border border-border overflow-hidden">
                <button
                  ...
                  className={[
                    'px-2 py-1 text-xs font-medium transition-colors border-r border-border',
                    draftMapColorMode === 'cluster'
                      ? 'bg-link text-white'
                      : 'bg-bg-accent hover:bg-bg-active text-text-secondary',
                  ].join(' ')}
                >
                  Cluster
                </button>
            ...
            <>
              <p className="text-xs text-text-secondary leading-snug max-w-2xl">
                <span>
                  ...
                  granularity; higher values produce more, smaller clusters.{' '}
                  <span className="opacity-60">
                    <strong>Neighbors (k)</strong> controls graph connectivity; higher values produce smoother
                    boundaries.
                  </span>
                </span>
              </p>
      ...
      <CardBody>
        <div className="grid grid-cols-2 gap-6 items-start">
          <div
            className={`relative ${showDraftMapScopePanel ? '' : 'col-span-2'}`}
            style={{
              aspectRatio: '1 / 1',
              width: showDraftMapScopePanel ? '100%' : 'calc(50% - 0.75rem)',
              ...(!showDraftMapScopePanel ? { margin: '0 auto' } : {}),
            }}
          >
            {clusteringInProgress && (
              <div
                className={`${draftMapPoints.length === 0 ? '' : 'absolute inset-0 bg-bg/60 backdrop-blur-sm'} z-10 flex items-center justify-center rounded`}
            ...
            (() => {
              ...
              const allCardImages = [
                ...selectedCards.map((c) => ({
                  ...
                  name: c.name,
                  imageUrl: displayRunData.cardMeta[c.oracle_id]?.imageUrl ?? '',
                })),
                ...selectedDeckCards.map((c) => ({
                  oracleId: c.oracle_id,
                  name: c.name,
                  imageUrl: displayRunData.cardMeta[c.oracle_id]?.imageUrl ?? '',
                })),
              ...
              const cardInfo: MapSelectedCardInfo | undefined =
                mapPanelHasBoth && hasAnyCardFilter
                  ? {
                      cardImages: allCardImages,
                      name: buildCardFilterLabel(),
                      pickRate:
                        selectedCards.length === 1 && selectedDeckCards.length === 0
                          ? (statsForScope?.pickRate ?? selectedCard?.pickRate)
                          : undefined,
                      avgPickPosition:
                        selectedCards.length === 1 && selectedDeckCards.length === 0
                          ? (statsForScope?.avgPickPosition ?? selectedCard?.avgPickPosition)
                          : undefined,
                      onClear: () => {
                        onClearSelectedCards();
                        onClearSelectedDeckCards();
                ...
                <div className="min-w-0 flex flex-col gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {selectedCardScopeLabel && (
                            <div className="mt-0.5">
                              <Text xs className="text-text-secondary/80">
                                {detailedViewTitle}
                              </Text>
                            </div>
                          )}
                          ...
                          {statsForScope && selectedCards.length === 1 && selectedDeckCards.length === 0 && (
                            <Flexbox direction="row" gap="3" alignItems="center" className="flex-wrap mt-1.5">
                              <span className="text-xs text-text-secondary/50 font-medium">
                                Pick rate{' '}
                                <span className="text-text-secondary/80">{`${(statsForScope.pickRate * 100).toFixed(1)}%`}</span>
                              </span>
                              <span className="text-xs text-text-secondary/50 font-medium">
                                Avg position{' '}
                                <span className="text-text-secondary/80">
                                  {statsForScope.avgPickPosition > 0 ? statsForScope.avgPickPosition.toFixed(1) : '—'}
                                </span>
                              </span>
                            </Flexbox>
                          )}
                  ...
                  {selectedSkeletonId !== null ? (
                    (() => {
                      ...
                      const clusterDecks =
                        sk && activeDecks ? sk.poolIndices.map((i) => activeDecks[i]).filter(Boolean) : null;
                      return sk ? (
                        <ClusterDetailPanel
                          skeleton={sk}
                          clusterIndex={skIdx}
