...
export abstract class BaseDynamoDao<T extends BaseObject, U extends BaseObject = T> {
    ...
    private async getRaw(key: Key): Promise<DynamoItem<U> | undefined> {
        try {
            ...
            const code =
                error instanceof DaoError
                    ? error.code
                    : ErrorCode.INTERNAL_SERVER_ERROR;

            throw new DaoError(
                `Error getting item with key: ${error.message}`,
                code,
     ...
     */
    protected async updateWithOptimisticLocking(
        item: T,
        expectedDynamoVersion: number
    ): Promise<void> {
        const dynamoItem = this.toDynamoItem(item);
    ...
    protected async queryByHash(
        ...
        const items = await Promise.all(
            itemPKs.map((itemPK) => this.get({ PK: itemPK, SK: this.itemType() }))
        );
