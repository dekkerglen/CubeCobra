...
export class CommentDynamoDao extends BaseDynamoDao<Comment, UnhydratedComment> {
     ...
     */
    private createHydratedComment(
        document: UnhydratedComment,
        owner: User,
        image: CubeImage
    ): Comment {
        return {
    ...
    private createHydratedCommentWithoutOwner(item: UnhydratedComment): Comment {
        return this.createHydratedComment(
            item,
            this.getAnonymousUser(),
            getImageData('Ambush Viper')
        );
    }
    ...
    public async delete(item: Comment): Promise<void> {
        if (this.dualWriteEnabled) {
            ...
            await Promise.all([
                CommentModel.delete({ id: item.id }),
                super.delete(item),
            ]);
        } else {
