...
router.get('/explore', async (req, res) => {
  ...
  const popular = (await Cube.batchGet(popularHashes.items.map((hash) => hash.cube))).filter(
    (cube) => cube.visibility !== 'pr',
  );

  const recentDecks = await Draft.queryByTypeAndDate(Draft.TYPES.DRAFT);
  const recentlyDrafted = (await Cube.batchGet(recentDecks.items.map((deck) => deck.cube))).filter(
    (cube) => cube.visibility !== 'pr',
  );
