import Card, { BoardChanges, Changes, CubeCardChange, CubeCardEdit, CubeCardSwap } from '@utils/datatypes/Card';
    ...
    for (const change of changes) {
      ...
      boardChange.removes?.forEach((rem: CubeCardChange) => {
        ...
        for (let i = addOps.length - 1; i >= 0; i--) {
          const addOp = addOps[i];
          if (addOp && addOp.card.index === idx) {
            addOps.splice(i, 1);
            foundAdd = true;
          }
        }
      ...
      boardChange.edits?.forEach((edit: CubeCardEdit) => {
        ...
        for (let i = 0; i < addOps.length; i++) {
          const addOp = addOps[i];
          if (addOp && addOp.card.index === idx) {
            if (currentVersion >= addOp.version) {
