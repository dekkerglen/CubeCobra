...
export const cardManaSymbols = (card: Card): ManaSymbol[] => {
  ...
  return cost
    .map((part: string) => {
      ...
      // Return undefined for parts that don't match any condition
      return undefined;
    })
    .flat()
...
export function sortInto(card: Card, result: Card[][][]) {
  ...
  if (!result[row]) {
    result[row] = new Array(8).fill(null).map(() => []);
  }
...
export function sortDeck(deck: (any | any[])[]): any[][] {
  const result = [new Array(8).fill(null).map(() => []), new Array(8).fill(null).map(() => [])];
